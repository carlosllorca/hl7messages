...
use carlosllorca\hl7messages\Scenarios\Immunization;
...
class Hl7v2 extends \yii\base\Component
{
     ...
     * Load Methods
     */
    public function loadImmunizationMessage($file){
        $fp = fopen($file, 'r');
        $linea = fgets($fp);
        $arr=$this->stringSplit($linea,$this->_hl7Globals['FIELD_SEPARATOR']);
        if($arr[0]!='MSH'){
            throw new ErrorException('Not found MSH segment');
        }
        if($arr[8]!='VXU^V04^VXU_V04'){
            throw new ErrorException('The file be for Immunization scenary in line 1');
        }
        ...
        while(!feof($fp)) {
            ...
            if($linea!=''){
                ...
                if($arr[0]=='PID'){
                    ...
                    $immunization->pid->loadPidSegment($arr);
                    $new=false;

                }elseif($arr[0]=='ORC'){
                    if($immunization){
                        $immunization->saveVaccine();
                    }
                    $immunization->createVaccine();
                    //$immunizacion->temp_vaccine->saveSegment($this->loadOrcSegment($arr));
                    $immunization->temp_vaccine->orc->loadFromField($arr);

                }elseif($arr[0]=='RXA'){
                    $immunization->temp_vaccine->rxa->loadFormField($arr);
                }elseif($arr[0]=='RXR'){
                    $immunization->temp_vaccine->rxr->loadFormField($arr);
                }elseif($arr[0]=='NK1'){
                    $immunization->nk1_temp->loadFromField($arr);
                    $immunization->saveNk1();
                }else{
                    echo json_encode($arr);
                    throw new ErrorException('Unknown Segment '.$arr[0]." in line ".$line);
                }
            }
            $line++;
        }
